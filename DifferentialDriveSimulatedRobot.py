...
class DifferentialDriveSimulatedRobot(SimulatedRobot):
    ...
    def __init__(self, xs0, map=[],*args):
        ...
        * **Rfp** : Object attribute containing the covariance of the simulated Polar2D feature noise (:math:`R_{fp}=diag(\\sigma_{\\rho}^2,\\sigma_{\\phi}^2)`)
        ...
        self.Cartesian2D_feature_reading_frequency = 50 # frequency of Cartesian2D feature readings
        ...
        self.xy_feature_reading_frequency = 0  # frequency of XY feature readings
        self.xy_max_range = 0  # maximum XY range, used to simulate the field of view

        self.yaw_reading_frequency = 100000  # frequency of Yasw readings
    ...
    def fs(self, xsk_1, usk):  # input velocity motion model with velocity noise
        ...
        **To be completed by the student**.
        ...
        # TODO: to be completed by the student
           ...
           # from pdf
        # http://127.0.0.1:5500/docs/build/html/robot_simulation.html#DifferentialDriveSimulatedRobot.DifferentialDriveSimulatedRobot.fs

        
        K = np.diag(np.abs(np.random.randn(3))) # 3x3

        # for position and velocity in x, y and z axis
        xsk = np.zeros((6, 1)) 
        ...
        prev_pose = Pose3D(np.array([[xsk_1[0, 0]], [xsk_1[1, 0]], [xsk_1[2, 0]]]))  #prev_pose=eta_sk_1
        prev_vel = Pose3D(np.array([[xsk_1[3, 0]], [xsk_1[4, 0]], [xsk_1[5, 0]]]))  #prev_vel=nu_sk_1

        # usk=nu_d=[u_d r_d]
        # desired velocity = nu_d
        desired_vel = Pose3D(np.array([[usk[0, 0]], [0] ,  [usk[1, 0]]])) 
        
        # current pose (eta_sk) and velocity (nu_sk) 
        current_pose= prev_pose.oplus(Pose3D(np.array(prev_vel * self.dt + 0.5 * wsk * self.dt*self.dt))) 
        current_velocity = prev_vel + K@(desired_vel - prev_vel) + wsk * self.dt 
        
        self.usk = desired_vel
        
        # current robot state for plotting purpose
    ...
    def ReadEncoders(self):
        ...
        **To be completed by the student**.
        ...
        zsk = np.array([[nL],[nR]]) + np.random.multivariate_normal([0,0],self.Re).reshape(2,1)
        Rsk = self.Re
        return zsk, Rsk
    ...
    def ReadRanges(self):
        ...
        """ Compute the distances from the robot to the landmarks.
        returns: the distance to each landmark
        """
        
        # TODO: to be completed by the student

        # map = self.M = coming from SimulatedRobot
        # self.xsk[:2] = current robot position (xsk) = slice (x,y)
        # np.linalg.norm = for euclidean distance
        
        noise_std=1

        landmarks_dist = []

        for landmark in self.M:
            distance = np.linalg.norm(self.xsk[:2] - landmark)

            # out of map  
            if distance > self.xy_max_range:
                continue
            
            # random noise with normal function =
            # mean=0, std= 0.5 

            noise = np.random.normal(0, noise_std)

            landmarks_dist.append((landmark, distance+noise))

        return landmarks_dist, noise_std
