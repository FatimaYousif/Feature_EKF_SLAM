...
class FEKFMBL(GFLocalization, MapFeature):
    ...
    def h(self, xk):  # overloaded to stack measurements and feature observations
        ...
        * Feature Observations: :math:`z_f` correspond to observations of the features (CartesianFeature, PolarFeature, EsphericalFeature, etc...).
        ...
        if self.headingData == True or self.encoderData == True:
            hm = self.hm(xk)
        else:
            hm = np.zeros((0,1))
    ...
    def ICNN(self, hf, Phf, zf, Rf, dim):
        ...
        if zf is not None:
            for zfi, Rfi in zip(zf, Rf):
                ...
                D2_min = np.inf
                for i in range(self.nf):
                    D2_ij = self.SquaredMahalanobisDistance(hf[i], Phf[i], zfi, Rfi)
    ...
    def SplitFeatures(self, xk, zf, Rf, H):
        ...
        for i in range(len(H)):
            ...
            if j is not None:
                ...
                # print(j)
                Hp = np.block([[Hp], [self.Jhfjx(xk, j)]])

                Vp = scipy.linalg.block_diag(Vp, np.diag(np.ones(self.zfi_dim)))
            else:
    ...
    #     if zp is None:
    #         # print("No features")
    #         return zm, Rm, Hm, Vm, znp, Rnp
        
    #     zk = np.concatenate([zm, zp])
    #     Rk = scipy.linalg.block_diag(Rm, Rp)
    #     Hk = np.concatenate([Hm, Hp])
    #     Vk = scipy.linalg.block_diag(Vm, Vp)
    ...
    #         if j is None:
    #             znp = np.concatenate((znp, zfi)) if znp is not None else zfi
    #             Rnp = scipy.linalg.block_diag(Rnp, Rfi) if Rnp is not None else Rfi
    #             continue
        
            
    #         zp = np.concatenate((zp, zfi)) if zp is not None else zfi
    ...
    #         Vp = scipy.linalg.block_diag(Vp, eye) if Vp is not None else eye
